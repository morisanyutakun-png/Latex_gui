    ...
    "function_declarations": [
        ...
        },
        {
            ...
            "description": (
                ...
                "IMPORTANT: For design changes, custom layouts, or anything beyond simple blocks, "
                "use type='latex' blocks to write raw LaTeX code directly. "
                            ...
                            "properties": {
                                ...
                                "block": {
                                    ...
                                    "description": (
                                        ...
                                        "The 'latex' type accepts raw LaTeX code: {type: 'latex', code: '\\\\begin{tcolorbox}...\\\\end{tcolorbox}'}"
                                    ),
                                },
                                "blockId": {
                                    "type": "string",
...
- **ブロックスタイルの fontSize は 10〜14 の範囲にせよ（デフォルト12）。見出しのサイズは heading の level (1/2/3) で制御し、fontSize で大きくするな**
- **fontFamily は "sans" をデフォルトにせよ（教材に適したゴシック体）**

### Step 4: 検証（必須 — 省略するな）
- **`compile_check` を必ず呼んで**LaTeX構文エラーがないか確認
- エラーがあれば `edit_document` で自動修正し、再度 compile_check
- **特に diagram/circuit/chart ブロックは TikZ エラーが起きやすいので compile_check 必須**

### Step 5: 品質確認（必須 — 省略するな）
- compile_check 通過後、**`read_document` で文書全体を再度読み取り**、以下を確認:
  - ブロックの順序が論理的か
  - 見出しレベルが適切か（H1が全体タイトル、H2がセクション、H3がサブセクション）
  - 数式の LaTeX 記法が正しいか
  - 問題文・選択肢・解答欄のフォーマットが統一されているか
- 問題があれば `edit_document` で修正し、再度 compile_check → read_document

### Step 6: 報告
- 何をしたか、どのような構成で文書を作成したかを**3〜5行**で報告
- 作成した内容の概要（例: 「第1問〜第3問の等差数列問題を追加しました。各問は基本・応用・発展の3段階です。」）

**重要: Step 1〜5 は全てツール呼び出しで自律的に行え。特に Step 4 と Step 5 は品質保証のために必ず実行すること。ユーザーに確認を求めるな。**
...
| **latex** | **生のLaTeXコード** | **code** (そのままLaTeX出力に埋め込まれる) |

### ★ latex ブロック — デザインの自由度を最大化する最重要ブロック
**デザインの変更、カスタムレイアウト、装飾、色付きボックス、マルチカラム等は全て latex ブロックで実現せよ。**

例: 色付きボックス
```json
{"op": "add_block", "afterId": null, "block": {
  "id": "ai-box-1",
  "content": {"type": "latex", "code": "\\begin{tcolorbox}[colback=blue!5,colframe=blue!50,title=ポイント]\n重要な内容をここに書く\n\\end{tcolorbox}"},
...
例: カスタムヘッダー
```json
{"type": "latex", "code": "\\begin{center}\n{\\Huge\\bfseries\\color{blue!70} 期末テスト}\n\\vspace{0.5cm}\n{\\large 数学I — 第3章 二次関数}\n\\end{center}\n\\hrule\\vspace{0.3cm}"}
```

### update_advanced — プリアンブルを変更してパッケージ追加・マクロ定義
latex ブロックで tcolorbox, multicol 等を使う場合、プリアンブルに \\usepackage を追加する必要がある:
```json
{"op": "update_advanced", "advanced": {
  "customPreamble": "\\usepackage{tcolorbox}\n\\usepackage{multicol}\n\\usepackage{xcolor}"
}}
```
...
1. **必ず latex ブロックで生のLaTeXコードを書け。heading/paragraph等の定型ブロックだけで済ませるな。**
2. `update_advanced` で必要なパッケージ(tcolorbox, fancyhdr, geometry, xcolor, multicol等)をプリアンブルに追加
3. tcolorbox で色付きボックス、\\vspace/\\hspace で余白調整、\\Large/\\Huge/\\bfseries でフォント変更、等を駆使
4. 既存の heading/paragraph ブロックを delete_block で削除し、latex ブロックで洗練されたデザインに置き換えることも積極的に行え

**例: 「かっこよくして」と言われたら:**
```
1. update_advanced: \\usepackage{tcolorbox}\\n\\usepackage{fancyhdr}\\n\\usepackage{xcolor}
2. 既存のheadingブロックを削除
3. latex ブロックで \\begin{center}{\\Huge\\bfseries\\color{blue!70} タイトル}\\end{center} を追加
4. 区切り線を latex ブロックで \\noindent\\rule{\\textwidth}{0.4pt} に置き換え
5. 重要なセクションを tcolorbox で囲む
```
...
- **デザインに関する要望は必ず latex ブロック + update_advanced で対応せよ**
...
def _execute_compile_check(document: dict, args: dict) -> dict:
    """Execute compile_check tool — validate document structure and LaTeX syntax."""
    try:
        ...
        for i, block in enumerate(blocks):
            ...
            if isinstance(fs, (int, float)) and (fs > 24 or fs < 6):
                block_issues.append(f"ブロック{i+1}({btype}): fontSize={fs} が異常です（推奨: 10〜14）")

            # heading ブロックのテキスト空チェック
            if btype == "heading" and not content.get("text", "").strip():
                block_issues.append(f"ブロック{i+1}: 空の見出しがあります")

            # paragraph ブロックの空チェック
            if btype == "paragraph" and not content.get("text", "").strip():
                block_issues.append(f"ブロック{i+1}: 空の段落があります")
        ...
        return {
            ...
            "message": f"構文チェック OK（{len(blocks)}ブロック, {len(latex_source)}文字）" if not issues else f"{len(issues)}件の問題を検出",
        }
