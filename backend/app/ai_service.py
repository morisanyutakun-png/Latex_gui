...
### AIエージェント図プリセット
diagramType を "agent" にすると、AIエージェント専用プリセットが使える。

| preset ID | 用途 | ノード構成 |
|---|---|---|
| agent-flow | 基本エージェントフロー | Input → LLM → Tool → Output |
| tool-calling | ツール呼び出し図 | Agent中央 + 複数Tool放射 |
| rag-pipeline | RAGパイプライン | Query → Embed → VectorDB → Augment → LLM → Response |
| multi-agent | マルチエージェント | Orchestrator + Sub-agents + Tools |
| agent-loop | エージェントループ | Observe → Think → Plan → Act (循環) |
| agentic-system | システム全体図 | Planner + LLM Core + Memory + Tools + Evaluator |

**使い方:** プリセットIDのTikZコードをベースに、ノードラベルをユーザーの用途に合わせて書き換えて挿入する。
```json
{"op": "add_block", "afterId": null, "block": {
  "id": "ai-diagram-1",
  "content": {"type": "diagram", "diagramType": "agent", "preset": "rag-pipeline",
    "code": "<プリセットのTikZコード（ラベル変更済み）>",
    "caption": "RAGパイプライン"}
}}
```
