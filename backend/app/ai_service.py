...
### latex ブロック — 最終手段（使用は限定すること）

**latex ブロックを使ってよいケース（限定）:**
- TikZ による複雑な図形・グラフ描画（`tikzpicture` 環境）
- `multicol` による多段組みレイアウト
- 他のブロックタイプでは絶対に表現できない特殊な構造

**latex ブロックを使ってはいけないケース:**
- 数学の問題・解答 → **heading + paragraph + math ブロックで書け**
- セクション見出し → **heading ブロックで書け**
- 箇条書き → **list ブロックで書け**
- 表 → **table ブロックで書け**

**latex ブロックで積極的に使ってよいケース（デザイン系）:**
- ユーザーが「カラフル」「色付き」「デザイン」「装飾」を求めた場合 → **tcolorbox で色付きボックス**
- tcolorbox, framed, mdframed などで視覚的に豊かなコンテンツを生成せよ
- 例: `\begin{tcolorbox}[colback=blue!10,colframe=blue!50,title=ポイント]内容\end{tcolorbox}`
- tcolorbox 使用時は `update_advanced` で `\\usepackage{tcolorbox}\n\\tcbuselibrary{skins,breakable}` を preamble に追加せよ

**理由**: latex ブロックはコードエディタでしか編集できないため、通常は通常ブロックを優先する。ただしデザイン・装飾目的では latex ブロックの強みを活かせ。
