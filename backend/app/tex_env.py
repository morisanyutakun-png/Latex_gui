...
# ══════════════════════════════════════════════════════════════════
# 3. ウォームアップ (コマンド存在確認のみ — コンパイルしない)
#
# Koyeb Free (512MB) ではメモリが極めて限られる。
# Docker ビルド時に全キャッシュ構築済みなので、
# 起動時にコンパイルテストを行わず、コマンド存在のみ確認する。
# これにより起動時のメモリピーク (~300MB) を完全に回避。
# ══════════════════════════════════════════════════════════════════
...
def _run_warmup():
    """コマンド存在確認のみ。コンパイルテストは行わない (メモリ節約)。

    Docker ビルド時に包括的スモークテスト + appuser キャッシュ構築済みなので、
    ランタイムでのコンパイルテストは不要。
    512MB環境ではこのテストコンパイル自体がOOMの原因になる。
    """
    global LUALATEX_JA_OK, _lualatex_cache_warm

    logger.info("[warmup] Checking lualatex availability (no compile test — saving memory)...")

    if LUALATEX_AVAILABLE:
        LUALATEX_JA_OK = True
        _lualatex_cache_warm = True
        logger.info("[warmup] lualatex + luatexja available (build-verified). Cache assumed warm.")
    else:
