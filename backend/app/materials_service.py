...
MATERIALS_DB: list[dict[str, Any]] = [
    ...
    # ══════════════════════════════════════════
    {
        "id": "info_hs_public_key_crypto",
        "subject": "情報",
        "level": "高校2年",
        "topic": "公開鍵暗号（RSA）",
        "keywords": ["RSA", "公開鍵", "秘密鍵", "暗号", "復号", "オイラー", "トーシェント", "素数", "mod"],
        "problems": [
            {
                "type": "記述",
                "text": "RSA暗号の鍵生成、暗号化、復号化の各ステップについて、その目的と手順を簡潔に説明しなさい。ただし、数式を用いて説明する場合は、適切な記号（$p, q, n, \\phi(n), e, d, M, C$ など）を定義して使用すること。",
                "answer": "鍵生成: 大きな素数$p,q$を選び$n=pq$、$\\phi(n)=(p-1)(q-1)$を計算。$\\gcd(e,\\phi(n))=1$となる$e$を選び、$de \\equiv 1 \\pmod{\\phi(n)}$となる$d$を求める。公開鍵$(n,e)$、秘密鍵$(n,d)$。暗号化: $C \\equiv M^e \\pmod{n}$。復号化: $M' \\equiv C^d \\pmod{n}$。",
                "hint": "鍵生成の目的は公開鍵と秘密鍵のペアを作ること。暗号化は誰でも公開鍵で暗号文を作れること。復号化は秘密鍵の所有者だけが平文を復元できること。"
            },
            {
                "type": "計算",
                "text": "素数 $p=3$, $q=11$, 公開鍵の指数 $e=3$ のとき、$n$ と $\\phi(n)$ の値を求めなさい。",
                "answer": "$n = 33$, $\\phi(n) = 20$",
                "hint": "$n = pq$, $\\phi(n) = (p-1)(q-1)$",
                "latex": "n = 3 \\times 11 = 33,\\quad \\phi(n) = 2 \\times 10 = 20"
            },
            {
                "type": "計算",
                "text": "$\\phi(n) = 20$, $e = 3$ のとき、秘密鍵 $d$ を求めなさい。（$de \\equiv 1 \\pmod{\\phi(n)}$）",
                "answer": "$d = 7$",
                "hint": "$3d \\equiv 1 \\pmod{20}$ を満たす最小の正整数。$3 \\times 7 = 21 = 1 + 20$",
                "latex": "3 \\times 7 = 21 \\equiv 1 \\pmod{20}"
            },
            {
                "type": "計算",
                "text": "公開鍵 $(n,e) = (33,3)$ を用いて平文 $M=4$ を暗号化し、暗号文 $C$ を求めなさい。",
                "answer": "$C = 31$",
                "hint": "$C \\equiv 4^3 \\pmod{33}$。$4^3 = 64 = 33 \\times 1 + 31$",
                "latex": "C \\equiv 4^3 = 64 \\equiv 31 \\pmod{33}"
            },
            {
                "type": "計算",
                "text": "秘密鍵 $(n,d) = (33,7)$ を用いて暗号文 $C=31$ を復号化し、平文 $M'$ を求めなさい。",
                "answer": "$M' = 4$",
                "hint": "$M' \\equiv 31^7 \\pmod{33}$。繰り返し二乗法を使って計算する。$31 \\equiv -2 \\pmod{33}$ を利用。$(-2)^7 = -128 \\equiv -128 + 4 \\times 33 = 4 \\pmod{33}$",
                "latex": "M' \\equiv 31^7 \\equiv (-2)^7 = -128 \\equiv 4 \\pmod{33}"
            },
        ],
    },
    # ══════════════════════════════════════════
